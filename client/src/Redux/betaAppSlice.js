  ...
  async (betaApp) => {
    try {
      ...
      const { data } = await axios.get(
        `http://${import.meta.env.VITE_BACK_UR}:5000/betaApp`
      );
      return data;
    } catch (error) {
      console.log(error);
...
export const getmyBetaApp = createAsyncThunk("beta/myBetaApp", async () => {
  try {
    ...
    const { data } = await axios.get(
      `http://${import.meta.env.VITE_BACK_UR}:5000/betaApp/myapp`
    );
    return data;
  } catch (error) {
    console.log(error);
...
export const acceptBetaApp = createAsyncThunk("beta/acceptApp", async (id) => {
  try {
    ...
    const { data } = await axios.put(
      `http://${import.meta.env.VITE_BACK_UR}:5000/betaApp/accept/${id}`
    );
    return data;
  } catch (error) {
    console.log(error);
...
export const rejectBetaApp = createAsyncThunk("beta/rejectApp", async (id) => {
  try {
    ...
    const { data } = await axios.put(
      `http://${import.meta.env.VITE_BACK_UR}:5000/betaApp/reject/${id}`
    );
    return data;
  } catch (error) {
    console.log(error);
